...
def checks(config: Config):
    ...
    # Avoid dataloader warnings and potential slowdown when worker count is too high for the host.
    try:
        max_workers = len(os.sched_getaffinity(0))
    except Exception:
        max_workers = os.cpu_count() or config.num_workers
    if max_workers > 0 and config.num_workers > max_workers:
        logger.print_warning(f"num_workers={config.num_workers} is too high for this system; using {max_workers}")
