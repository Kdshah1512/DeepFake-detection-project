...
class DeepfakeDetectionModel(pl.LightningModule):
    ...
    def log_loss(self, loss: LossOutputs, stage: str, batch_size: int | None = None):
        if loss.total is not None:
            self.log(f"{stage}/loss", loss.total, prog_bar=True, on_epoch=True, batch_size=batch_size)
        if loss.ce_labels is not None:
            self.log(f"{stage}/loss_ce", loss.ce_labels, prog_bar=True, on_epoch=True, batch_size=batch_size)

    def log_aliunif(self, outputs: head.HeadOutput, labels: torch.Tensor, stage: str, batch_size: int | None = None):
        alignment = unifalign.alignment(outputs.features, labels)
        uniformity = unifalign.uniformity(outputs.features)
        self.log(f"{stage}/alignment", alignment, prog_bar=True, on_epoch=True, batch_size=batch_size)
        self.log(f"{stage}/uniformity", uniformity, prog_bar=True, on_epoch=True, batch_size=batch_size)
    ...
    def training_step(self, batch, batch_idx):
        ...
        batch_size = int(batch.labels.shape[0]) if batch.labels is not None else None
        self.log_loss(loss, "train", batch_size=batch_size)
        self.log_aliunif(outputs, batch.labels, "train", batch_size=batch_size)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        batch_size = int(batch.labels.shape[0]) if batch.labels is not None else None
        self.log_loss(loss, "val", batch_size=batch_size)
        self.log_aliunif(outputs, batch.labels, "val", batch_size=batch_size)
    ...
    def test_step(self, batch, batch_idx):
        ...
        batch_size = int(batch.labels.shape[0]) if batch.labels is not None else None
        self.log_loss(loss, "test", batch_size=batch_size)
        self.log_aliunif(outputs, batch.labels, "test", batch_size=batch_size)
