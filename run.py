...
def main(config: Config, train: bool):
    checks(config)
    log_path = logger.setup_run_logging(config.run_dir, config.run_name)
    logger.print_info(f"Run log file: {log_path}")
    ...
    try:
        ...
        data_module = datasets.DeepfakeDataModule(
            config,
            model.get_preprocessing()
        )
        ...
        if train:
            ...
            ckpt_path = (
                f"{config.run_dir}/{config.run_name}/checkpoints/"
                f"{config.checkpoint_for_testing}.ckpt"
            )
...
def get_train_config() -> Config:
    ...
    config.batch_size = config.mini_batch_size = 8
...
def get_test_config() -> Config:
    ...
    config.checkpoint = config_path.replace(
        "hparams.yaml",
        "checkpoints/best_mAP.ckpt"
    )
    config.wandb = False
